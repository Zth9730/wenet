...
#    autograd error
train_config=train_conformer_pinyin.yaml
...
average_num=30
decode_modes="attention" # ctc_greedy_search ctc_prefix_beam_search attention attention_rescoring
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  torchrun --nnodes=$num_nodes --nproc_per_node=$num_gpus \
    ...
    wenet/bin/train.py \
      ...
      --config $train_config \
      --data_type  $data_type \
      --train_data /data_new/wangyuwen/wenet/examples/aishell/s0/data/pinyin_aishell_kespeech/pinyin_aishell_kespeech_shuf.list \
      --cv_data /data_new/wangyuwen/wenet/examples/aishell/s0/data/pinyin_aishell_kespeech/aishell_dev_pinyin.list \
