...
class Conv2dSubsampling4V2(BaseSubsampling):
    """Convolutional 2D subsampling (to 1/4 length).
    ...
    def __init__(self, idim: int, odim: int, dropout_rate: float,
        ...
        super().__init__()
        self.conv = torch.nn.Sequential(
            torch.nn.LayerNorm(idim, eps=1e-6),
            torch.nn.Conv2d(1, odim, 3, 2),
            torch.nn.ReLU(),
            torch.nn.Conv2d(odim, odim, 3, 2),
            torch.nn.ReLU(),
        ...
        self.norm = torch.nn.LayerNorm(odim, eps=1e-6)
        self.post_layer = ConvolutionModule(odim,
                                           7,
                                           norm='layer_norm',
                                           norm_eps=1e-6)
    ...
    def forward(
        ...
        # residual = x
        # x = self.norm(x)
        # x, _ = self.post_layer(x, mask)
        # return self.dropout(x + residual), pos_emb, mask
